...
(function () {
    ...
    function UIManager(postfix, panels, hooks, undoManager, previewManager, commandManager, fullScreenManager, helpOptions, getString) {
        ...
        function makeSpritedButtonRow() {
            ...
            buttons.quote = makeButton("wmd-quote-button", getString("quote"), "-40px", bindCommand("doBlockquote"));
            buttons.code = makeButton("wmd-code-button", getString("code"), "-60px", bindCommand("doCode"));
            buttons.olist = makeButton("wmd-olist-button", getString("olist"), "-80px", bindCommand(function (chunk, postProcessing) {
                this.doList(chunk, postProcessing, true);
            }));
            buttons.ulist = makeButton("wmd-ulist-button", getString("ulist"), "-100px", bindCommand(function (chunk, postProcessing) {
                this.doList(chunk, postProcessing, false);
            }));
            buttons.heading = makeButton("wmd-heading-button", getString("heading"), "-120px", bindCommand("doHeading"));
            buttons.hr = makeButton("wmd-hr-button", getString("hr"), "-140px", bindCommand("doHorizontalRule"));
            buttons.more = makeButton("wmd-more-button", getString("more"), "-160px", bindCommand("doMore"));
            makeSpacer(2);

            buttons.link = makeButton("wmd-link-button", getString("link"), "-1800px", bindCommand(function (chunk, postProcessing) {
                return this.doLinkOrImage(chunk, postProcessing, false);
            }));
            buttons.image = makeButton("wmd-image-button", getString("image"), "-200px", bindCommand(function (chunk, postProcessing) {
                return this.doLinkOrImage(chunk, postProcessing, true);
            }));
            buttons.upload = makeButton("wmd-upload-button", getString("upload"), "-220px", null);
            buttons.box = makeButton("wmd-box-button", getString("box"), "-240px", null);
            makeSpacer(3);
                    ...
                    if (!chunk.selection) {
                        if (isImage) {
                            const getFileName = (url) => {
                                ...
                                return fileName || that.getString("imagename");
                            };

                            // var imagename = that.getString("imagename");    // 真实文件名
                            chunk.selection = getFileName(link) || that.getString("imagedescription");
                        }
                        else {
                            // var linkname = that.getString("linkname");
                            chunk.selection = link || that.getString("linkdescription");
                        }
                    }
                }
                postProcessing();
